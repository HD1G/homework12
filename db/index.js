...
class DB {
  ...
  findEmployeesByDepartment(department_id){
  ...
  findEmployeesByManager(manager_id){
  ...
  findAllManagers(){
  return this.connection.query
  (`SELECT employee.manager_id, CONCAT(manager.first_name, ' ', manager.last_name) AS manager FROM employee left join employee manager on manager.id = employee.manager_id GROUP BY manager.id;`);
  }
  
  findUnderlings(manager_id){
    return this.connection.query
    ("SELECT first_name, last_name FROM employee WHERE manager_id is not null and manager_id = ?", manager_id);
    }
}
